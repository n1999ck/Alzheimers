...
class RFC():
    def __init__(self):
        ...
        y_train_pred = self.rfc.predict(X_train)
        y_test_pred = self.rfc.predict(X_test)

        train_acc = accuracy_score(y_train,y_train_pred)
        RFC_test_accuracy = accuracy_score(y_test, y_test_pred)
        print(f"The RFC training {train_acc*100:.2f}%")
        print(f"The Random Forest model's accuracy on the testing dataset is: {RFC_test_accuracy*100:.2f}%")
...
#model3.eval(X_train, y_train, X_test, y_test)
...
#resources used ChatGPT, GeeksforGeeks, StackOverflow, DataScience.stackexchange
