...
class CSVDataset(Dataset):
    #Based on https://rumn.medium.com/how-to-create-a-custom-pytorch-dataset-with-a-csv-file-e64b89bc2dcc
...
actual = np.random.binomial(1, 0.9, size=1504)
predicted = np.random.binomial(1, 0.9, size=1504)
...
for epoch in range(num_epochs):
    ...
    print(
        f"Epoch: {epoch +1}/{num_epochs}, Loss: {total_loss/ len(dataset_loader):.4f}, LR: {scheduler.get_last_lr()[0]:.4f}"
    )
...
confusion_matrix = metrics.confusion_matrix(actual, allpredictions)
