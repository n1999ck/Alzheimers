...
# nn.Dropout # Commented out as it is not used directly
import pandas as pd
import numpy as np
...
# torch.optim.Adam # Commented out as it is not used directly

# modelLR = LogisticRegression(penalty='12') # Commented out as it's an incorrect parameter and replaced on 79
...
class FeedforwardNeuralNetModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        # self.sigmoid = nn.Sigmoid() # Commented out as it is not used
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        # Non-linearity
        # out = self.sigmoid(out) # Commented out as it is not used
        out = self.reLU(out)
...
# modelLR = LogisticRegression(solver='saga', max_iter=2000, C=0.5) # Commented out as it's redundant with the next line
...
# Normalizing data
# pipeline = make_pipeline(StandardScaler(), LogisticRegression(max_iter=2000)) # Commented out as pipeline is defined later
...
dataset_features_scaled = scaler.fit_transform(dataset_features) # This doesn't get used? Could be a break.
...
# scheduler = StepLR(optimizer, step_size=5, gamma=0.1) # Commented out as CosineAnnealingLR is used instead
...
val_split = 0.2  
val_size = int(len(train_portion_dataset) * val_split) # Fixed calculation for val_size
train_size = int(len(train_portion_dataset) * (1 - val_split))  # Small change.  Calculate directly based on the desired split with 1 - val_split. Also seems more clear.
...
for epoch in range(num_epochs):
    ...
    total = 0
    with torch.no_grad():
        for fields, labels in val_loader: #Validation loop
            fields, labels = fields.to(device), labels.to(device)
   ...
   #reset these before testing to be sure they only reflect the testing dataset, not validation + testing. 
    correct = 0
    total = 0

    for data in test_loader: #test loop.
        inputs, labels = data
        outputs = model(inputs)
        # outputs = model2(inputs) # Commented out as model2 is not defined
        _, predicted = torch.max(outputs.data, 1)
        total += labels.size(0) # Updated with the batch size.
        correct += (predicted == labels).sum().item() # Updated with correct predictions count.
